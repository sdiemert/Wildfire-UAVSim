...
class FollowFirePolicy(Policy):
    """Fly toward the nearest visible fire, hold position when no fire is in view.

    Only the observed cells are consulted, so a UAV with no fire inside its UAV_OBSERVATION_RADIUS window
    stays put rather than searching. Movement is along a single axis, at whatever speed closes the gap on
    that axis without overshooting, so the UAV approaches diagonal targets in a staircase.
    ...
    def action_for(self, observation):
        ...
        if target is None:  # nothing visible, hold position
            return Action.stay()
        # step_towards() returns Action.stay() when the UAV is already above the fire
