...
# able to compare the managing systems -- and the unmanaged baseline -- without editing a file and
# restarting the server. Its choices come from the registry in sim/managing/systems.py, so a managing
# system added there appears in the dropdown without this module changing. AdaptiveWildFireModel takes both
# of these as per model overrides and never writes them back to config, so a Reset with different settings
# builds a different model rather than changing the configuration for everything built afterwards.
#
...
def model_params():
    return {
        "managing": mesa.visualization.Choice(
            ...
            choices=sorted(MANAGING_SYSTEMS),
            description="Which MAPE-K managing system runs over the simulation, reallocating a policy to "
                        "each UAV as the run goes. 'none' is the unmanaged baseline: every UAV flies the "
                        "policy below for the whole run. The rest are combinations of the five MAPE-K "
                        "components, registered in sim/managing/systems.py -- including 'remote', which "
                        "runs the whole loop on the server at MANAGING_SYSTEM_URL and keeps only the "
                        "sensor and the effector here. The panel on the left says what the selected one "
                        "is made of. Overrides MANAGING_SYSTEM in config.py for this run only. Press "
                        "Reset to apply.",
        ),
...
def main():
    ...
    print('managing systems:', ', '.join(sorted(MANAGING_SYSTEMS)))
